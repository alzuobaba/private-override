#!/usr/bin/env python3
"""Full deobfuscation pipeline for jsjiami v5 scripts - v2 with better body extraction."""
...
def find_body_start(result: str) -> int:
    """Find where the main script body starts (after the decoder function).
    Common patterns:
      - var _0x...=$response[  (legacy 7dong style)
      - if($request[...         (modern LuckPik style)
      - if(typeof _0x...==="string"   (string check)
      - (function($request      (IIFE wrap)
    """
    patterns = [
        r'(var\s+_0x[0-9a-f]+\s*=\s*\$response\[)',
        r'(var\s+_0x[0-9a-f]+\s*=\s*\$request\[)',
        r'(if\s*\(\s*\$request\[)',
        r'(if\s*\(\s*typeof\s+_0x)',
        r'(if\s*\(\s*\$response\[)',
    ]
    earliest = -1
    for p in patterns:
...
def find_body_end(result: str, body_start: int) -> int:
    """Find where the main script body ends (before debug-protection wrapper).
    Common end markers:
      - ;setInterval(   (setInterval for debugger loop)
      - ;(function(_0x  (debugger IIFE)
      - function _0x...{  (debugger function definition)
      - encode_version = 'jsjiami.com.v5'  (the marker line)
    """
    candidates = []
...
if __name__ == '__main__':
    if len(sys.argv) < 2:
        print("Usage: python3 jsjiami_v5_decode_v2.py <input.js> [output.js]")
        sys.exit(1)
